...
`muer-showcase` is a consuming Spring Boot application, not another IAM
...
```bash
export MUER_SHOWCASE_JDBC_URL='jdbc:mysql://127.0.0.1:3306/iam_example'
export MUER_SHOWCASE_DB_USERNAME='iam'
export MUER_SHOWCASE_DB_PASSWORD='replace-with-local-password'
...
```bash
export MUER_SHOWCASE_BASE_URL='http://127.0.0.1:8080'
```
...
LOGIN_RESPONSE="$(curl --fail-with-body -sS \
  -X POST "$MUER_SHOWCASE_BASE_URL/iam/auth/login" \
  -H 'Content-Type: application/json' \
  --data '{"username":"operator-a","password":"demo-pass","clientType":"WEB"}')"
export MUER_SHOWCASE_TOKEN="$(jq -r '.accessToken' <<<"$LOGIN_RESPONSE")"
export MUER_SHOWCASE_SESSION_ID="$(jq -r '.sessionId' <<<"$LOGIN_RESPONSE")"
```
...
```bash
curl --fail-with-body -sS "$MUER_SHOWCASE_BASE_URL/iam/auth/me" \
  -H "Authorization: Bearer $MUER_SHOWCASE_TOKEN"

curl --fail-with-body -sS "$MUER_SHOWCASE_BASE_URL/iam/sessions" \
  -H "Authorization: Bearer $MUER_SHOWCASE_TOKEN"
```

The session list includes `MUER_SHOWCASE_SESSION_ID`; MySQL remains the durable
...
```bash
curl --fail-with-body -sS "$MUER_SHOWCASE_BASE_URL/example/orders/9001" \
  -H "Authorization: Bearer $MUER_SHOWCASE_TOKEN"
```
...
curl -sS -o /dev/null -w '%{http_code}\n' \
  "$MUER_SHOWCASE_BASE_URL/example/orders/9002" \
  -H "Authorization: Bearer $MUER_SHOWCASE_TOKEN"
# 403
```
...
```bash
curl -sS -o /dev/null -w '%{http_code}\n' \
  -X POST "$MUER_SHOWCASE_BASE_URL/example/orders/9001/approve" \
  -H "Authorization: Bearer $MUER_SHOWCASE_TOKEN"
# 403
```
...
SWITCH_RESPONSE="$(curl --fail-with-body -sS \
  -X POST "$MUER_SHOWCASE_BASE_URL/iam/authorization/profiles/402/switch" \
  -H "Authorization: Bearer $MUER_SHOWCASE_TOKEN")"
export MUER_SHOWCASE_APPROVER_TOKEN="$(jq -r '.accessToken' <<<"$SWITCH_RESPONSE")"
export MUER_SHOWCASE_APPROVER_SESSION_ID="$(jq -r '.sessionId' <<<"$SWITCH_RESPONSE")"

curl --fail-with-body -sS \
  -X POST "$MUER_SHOWCASE_BASE_URL/example/orders/9001/approve" \
  -H "Authorization: Bearer $MUER_SHOWCASE_APPROVER_TOKEN"
```
...
```bash
curl --fail-with-body -sS \
  -X POST "$MUER_SHOWCASE_BASE_URL/iam/authorization/diagnostics" \
  -H 'Content-Type: application/json' \
  -H "Authorization: Bearer $MUER_SHOWCASE_TOKEN" \
...
curl --fail-with-body -sS -o /dev/null -w '%{http_code}\n' \
  -X POST "$MUER_SHOWCASE_BASE_URL/iam/sessions/$MUER_SHOWCASE_APPROVER_SESSION_ID/revoke" \
  -H 'Content-Type: application/json' \
  -H "Authorization: Bearer $MUER_SHOWCASE_APPROVER_TOKEN" \
  --data '{}'
# 204
...
curl -sS -o /dev/null -w '%{http_code}\n' \
  "$MUER_SHOWCASE_BASE_URL/example/orders/9001" \
  -H "Authorization: Bearer $MUER_SHOWCASE_APPROVER_TOKEN"
# 401
```
